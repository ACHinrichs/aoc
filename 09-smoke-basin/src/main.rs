    ...
    for i in 0..ground.len() {
        ...
        for j in 0..ground[i].len() {
            if ground[i][j] == 9 {
                ...
                let dir_array: [(i64, i64); 4] = [(0, 1), (0, -1), (1, 0), (-1, 0)];
                for direction in dir_array {
                    if 0 <= (i as i64) + direction.0
                        ...
                        && ground[(i as i64 + direction.0) as usize]
                            ...
                            < ground[(i as i64 + flow.0) as usize][(j as i64 + flow.1) as usize]
                    {
                        flow = direction;
                    }
                }
    ...
    while !to_explore.is_empty() {
        ...
        for dir in dir_array {
            if 0 <= (cur.0 as i64) + dir.0
                ...
                && flow_matrix[(cur.0 as i64 + dir.0) as usize][(cur.1 as i64 + dir.1) as usize]
                    == (-dir.0, -dir.1)
            {
                println!("{:?} + {:?}", cur, dir);
                to_explore.push((
                    (cur.0 as i64 + dir.0) as usize,
