...
impl Vent{
	...
	for i in 0..self.length(){
		...
		(other.is_straight() ||
		 ( (other.start[0] - x).abs() == (other.start[1] - y).abs()
		 && (other.end[0] - x).abs() == (other.end[1] - y).abs())
		 )
	    {
	...
	let p1 = split[0].split(",").map(|x| x.parse::<i64>().unwrap_or(0)).collect::<Vec<i64>>();
	let p2 =split[1].split(",").map(|x| x.parse::<i64>().unwrap_or(0)).collect::<Vec<i64>>();
    ...
    fn length(&self) -> i64{
	std::cmp::max(self.end[0]-self.start[0], self.end[1]-self.start[1]) + 1
    }
