...
fn part2(numbers: Vec<i32>, mut boards: Vec<Vec<Vec<i32>>>){
	...
	for i in 0..boards.len(){
		...
		finished_order.push((copy_board(&boards, i), numbers[number_index]));
	    } else {
    ...
    println!("Winning-Sum * last number is {}", winning_sum * finished_order[0].1);

    let loosing_sum = collect_positive(&finished_order[finished_order.len()-1].0);
    println!("Loosing-Sum is {:?}", loosing_sum);
    println!("Loosing-Sum * last number is {}", loosing_sum * finished_order[finished_order.len()-1].1);
}

fn collect_positive(board: &Vec<Vec<i32>>) -> i32{
    (*board).iter().map(|y| y.iter().fold(0, |b, x| b + cmp::max(x,&0)))
	.fold(0, |b, x| b + cmp::max(x,0))
}

fn copy_board(boards: &Vec<Vec<Vec<i32>>>, i: usize) -> Vec<Vec<i32>>{
    boards[i].iter().map(|x| x.iter().map(|x| *x).collect::<Vec<i32>>()).collect::<Vec<Vec<i32>>>()
}
