...
fn main() {
	...
	for pattern in signal_patterns{
	    let mut sorted_pattern = pattern.chars()
		.collect::<Vec<char>>();
	    sorted_pattern.sort();
	    println!("{:?}", pattern);
	...
	while yet_unknown.len() > 0{
		...
		if sorted_contains(&sorted_pattern,
				   mapping.get(&4).unwrap()){
		...
		if sorted_contains(&sorted_pattern,
		    ...
		    // and cyclic bad
		    if sorted_contains(mapping.get(&9).unwrap(),
				       &sorted_pattern){
	...
	for i in 0..=9{
	    println!("{} {:?}",i,mapping.get(&i).expect("That really should be there"));
	    scrambled_to_number.insert(mapping.get(&i)
				       .expect("That really should be there")
				       //.into_iter()
				       //.collect::<String>()
				       ,
				       i);
...
fn sorted_contains(a :&Vec<char>, b :&Vec<char>) -> bool{
	...
	if a[i_a] == b[i_b]{//Halts at the first element in b not in a
	    i_b += 1; 
	}
    }
