...
fn main() {
	...
	let x_area = x_area_string[2..]
		.split("..")
		.map(|x| i64::from_str_radix(&x.to_string(), 10).unwrap())
		.collect::<Vec<i64>>();
	let y_area = y_area_string[2..]
		.split("..")
		.map(|x| i64::from_str_radix(&x.to_string(), 10).unwrap())
		.collect::<Vec<i64>>();
	println!("{:?}", y_area);
			...
			'steps_loop: loop {
					...
					&& y_pos <= y_area[1]
				{
					if y_max < this_y_max {
						...
						println!(
							"Current candidate is {} {}, with a y-max of {}",
							initial_x_vel, initial_y_vel, this_y_max
						);
					} else {
						println!("Found {} {}", initial_x_vel, initial_y_vel);
					}
					vel_counts += 1;
					break 'steps_loop;
				}
	...
	println!(
        ...
        y_max, winning_initial.0, winning_initial.1, vel_counts
    );
}
