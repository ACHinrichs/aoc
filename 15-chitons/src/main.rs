...
fn main() {
    let file = File::open("example.txt").expect("ERR: file not found!");
	...
	for j in 1..i{
		...
		+ std::cmp::max(res_vec[i][j],res_vec[i+1][j-1]);
	    res_vec[j][i+1] = input_numbers[i+1][j]
		+ std::cmp::max(res_vec[j][i],res_vec[j-1][i+1]);
	}
	// Here is a choice:
	res_vec[i+1][i+1] = input_numbers[i+1][i+1]
	    + std::cmp::max(res_vec[i+1][i],res_vec[i][i+1]);
