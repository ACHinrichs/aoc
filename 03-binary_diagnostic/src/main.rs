...
fn main() {
	...
	let lines = &BufReader::new(file)
		.lines()
		.map(|x| x.expect("ERR: while reading the File!"))
		.map(String::from)
		.collect::<Vec<String>>();
	...
	while oxyrating.len() > 1 && pos < *len {
		...
		oxyrating = (oxyrating.iter())
			.filter(|x| x[pos] == most_common)
			.map(|x| *x)
			.collect::<Vec<&Vec<u32>>>();
	...
	while co2rating.len() > 1 && pos < *len {
		...
		co2rating = (co2rating.iter())
			.filter(|x| x[pos] == least_common)
			.map(|x| *x)
			.collect::<Vec<&Vec<u32>>>();
	...
	println!(
		"combined: {}",
		bin_to_dec(oxyrating[0]) * bin_to_dec(co2rating[0])
	);
}
